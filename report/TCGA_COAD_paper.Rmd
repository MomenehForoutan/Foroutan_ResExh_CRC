...
## Survival plot for clinical factors
```{r}
sn <- "OS"

pdf(paste0(figPath, "OS_TCGA_annot.pdf"), height = 5, width = 4.2)
survival_plot(
  data =
...
# annFilt$MSI_Group[annFilt$MSI_TCGAbiolink_annot == "MSI-L"] <- "MSS"
...
survival_plot(
  ...
  scoreCol = NULL,
  gene = NULL,
  covariate = "MSI_TCGAbiolink_annot",
  # covariate = "MSI_Group",
  isCategoricalCov = TRUE,
  timeCol = paste0(sn, ".time"),
  eventCol = sn,
...
## IF including genes
```{r}
gg <- c("CD3E", "CD3G", "CD3D", "CD8A", "CD8B")

gg2 <- gg[gg %in% rownames(ruvTCGA_tum)]
markerExpr <- ruvTCGA_tum[gg2, ]

markerExprData <- data.frame(markerExpr, check.names = F, check.rows = F)
markerExprData$Group <- c("CD3", "CD3", "CD3", "CD8", "CD8")
...
row.names(markerExprData) <- markerExprData$Group
markerExprData <- markerExprData[, ! colnames(markerExprData) == "Group"]

markerExprData <- markerExprData[, row.names(annFilt)]
...
#### Check Stage/CMS/MSI/Scores
```{r}
##--------- Cox for stage:

cox_stage <- coxph_test (
  data = scoreAnnot,
  timeCol = timeCol,
  eventCol = eventCol,
  # sigCol = "Signature",
  ...
  nStrata = 4,
  returnCovarStat = F,
  covarCol = c("Age", "CMS", "MSI_TCGAbiolink_annot"),
  returnCoxModels = F
)

cox_stage_noCovar <- coxph_test (
  data = scoreAnnot,
  timeCol = timeCol,
  eventCol = eventCol,
  # sigCol = "Signature",
...
cox_stage2 <- coxph_test (
  data = scoreAnnot,
  timeCol = timeCol,
  eventCol = eventCol,
  # sigCol = "Signature",
  variableCol = "Stage_2group",
  nStrata = 2,
  returnCovarStat = F,
  covarCol = c("Age", "CMS", "MSI_TCGAbiolink_annot"),
  returnCoxModels = F
...
cox_msi <- coxph_test (
  ...
  eventCol = eventCol,
  # sigCol = "Signature",
  variableCol = "MSI_TCGAbiolink_annot",
  nStrata = 3,
  returnCovarStat = F,
  covarCol = c("Age", "CMS", "Stage"),
  returnCoxModels = F
)

cox_msi_noCovar <- coxph_test (
  data = scoreAnnot,
  timeCol = timeCol,
  eventCol = eventCol,
  # sigCol = "Signature",
...
cox_cms <- coxph_test (
  ...
  nStrata = 4,
  returnCovarStat = F,
  covarCol = c("Age", "Stage", "MSI_TCGAbiolink_annot"),
  returnCoxModels = F
)

cox_cms_noCovar <- coxph_test (
  data = scoreAnnot,
  timeCol = timeCol,
  eventCol = eventCol,
  # sigCol = "Signature",
...
cox_Score <-
  coxph_multitest(
    ...
    nStrata = 2,
    returnCovarStat = F,
    covarCol = c("Age", "Stage", "CMS", "MSI_TCGAbiolink_annot"),
    nCores = 10
  )

## Scores without covariate
cox_Score_noCovar <-
  coxph_multitest(
    data = scoreAnnot,
    variableNameCol = "Signature",
...
Make Coef plots comparing teh hazard ratios.
```{r}
cox_stage$VariableName <- "Stage"
cox_msi$VariableName <- "MSI"
cox_cms$VariableName <- "CMS"

dd <- rbind(cox_stage, cox_msi, cox_cms, 
            cox_Score[grepl("Res_|Exh_|^CD3$|^CD8$", rownames(cox_Score)), ])

dd <- dd[, c("VariableName",
             "variable",
             "exp(coef)",
             "lower .95",
             "upper .95",
...
dd <- dd[! grepl("All_", dd$VariableName), ]

pdf(
  paste0(figPath, "OS_TCGA_Cox_Benchmark_Scores_Others.pdf"),
  height = 4.6,
  width = 5
)
...
#### Compare base model with added variables
Compre the base model with only 4 covariates with the models that have new score variables... none of the main signatures are significant... only WNT signalling
...
anRes <- sapply(cox_Score_models, function(x){
  anovaRes <- anova(base_model, x)[[4]][2]
  return(anovaRes)
})
...
pdf(paste0(figPath, "OS_TCGA_CD3.CD8_3groups.pdf"), height = 4.8, width = 4.2)
...
pdf(paste0(figPath, "OS_TCGA_CD3.CD8_averagedExpr.pdf"), height = 4.8, width = 4.2)

for(i in c("CD3", "CD8")){
  print(survival_plot(
  data =
    ruvTCGA_tum[, rownames(wide2)],
  stratify = "covariate",
  annot = wide2,
  scoreCol = NULL,
  gene = NULL,
...
**Looking within stages**
- NK resident signature is associated with poorer survival in stage III and IV
```{r}
cox_score_stageHigh <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$Stage %in% c("III", "IV"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_stageLow <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  & 
        scoreAnnot$Stage %in% c("I", "II"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
**Looking within MSI status**
- CD8_Exh is associated with better survival in MSI high
```{r}
cox_score_msiHigh <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$MSI_TCGAbiolink_annot %in% c("MSI-H"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_msiLow <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  & 
         scoreAnnot$MSI_TCGAbiolink_annot %in% c("MSI-L"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_mss <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  & 
         scoreAnnot$MSI_TCGAbiolink_annot %in% c("MSS"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_mss_msiLow <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  & 
         scoreAnnot$MSI_TCGAbiolink_annot %in% c("MSS", "MSI-L"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
**Looking within CMS status**
- CD8_Exh is associated with better survival in MSI high
```{r}
cox_score_cms1 <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$CMS %in% c("CMS1"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_cms2 <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$CMS %in% c("CMS2"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_cms3 <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$CMS %in% c("CMS3"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_score_cms4 <- coxph_multitest(
    data = scoreAnnot[
      grepl("Exh_|Res_", scoreAnnot$Signature)  &
        scoreAnnot$CMS %in% c("CMS4"),],
    variableNameCol = "Signature",
    variableCol = "Score_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
# saveRDS(wide2CombLong, paste0(outPath, "TCGA_Survival_OS_ScoreExpr_wide.RDS")) # previous version
```

#### IF adding stage and other scores
We consider MSI-L with MSS together.
We did not consider CMS as it left us with as low as 13/23 samples in some groups and some did not have CMS grouping.
```{r}
wide2Comb$MSI_Group[wide2Comb$MSI_TCGAbiolink_annot == "MSI-L"] <- "MSS"

addFactors <- c("Stage_2group", "MSI_Group", "CD8", "CD3")

for(a in addFactors){
  for(e in canSigs){
    currentColumn <- paste(a, e, sep = "--")
    wide2Comb[, currentColumn] <- paste(wide2Comb[, a], a , "&",
                                      wide2Comb[, e], e)
  }
} 
...
## replace NA.MSI -sigs and NA.Stage--sigs with NA and then remove them
wide2CombLong$Comb_2status[grepl("^NA ", wide2CombLong$Comb_2status)] <- NA
wide2CombLong <- wide2CombLong[complete.cases(wide2CombLong$Comb_2status), ]

wide2CombLong$OS.time <- wide2CombLong$OS.time/12
# saveRDS(wide2CombLong, paste0(outPath, "TCGA_Survival_OS_ScoreExpr_wide.RDS")) # new version



wide2CombLong_scores <- wide2CombLong[! grepl("MSI_Group--|Stage_2group--", wide2CombLong$Comb_Name), ]

wide2CombLong_msi <- wide2CombLong[grepl("MSI_Group--", wide2CombLong$Comb_Name), ]

wide2CombLong_stage <- wide2CombLong[grepl("Stage_2group--", wide2CombLong$Comb_Name), ]
```
...
cox_OS_2Comb_sig <-
  cox_OS_2Comb %>% filter(`Pr(>|z|)` < 0.05) %>%
  data.frame(check.names = F)

# NK_Res_Bulk--Mes_Thiery.currentCoxRes1               Low NK_Res_Bulk & High Mes_Thiery -1.135
# NK_Res_Bulk--TGFb_Signaling.currentCoxRes2        Low NK_Res_Bulk & Low TGFb_Signaling -0.754
# NK_Exh_Bulk--DDR.currentCoxRes2                              Low NK_Exh_Bulk & Low DDR  0.906
# NK_Exh_Bulk--Mes_Thiery.currentCoxRes2                Low NK_Exh_Bulk & Low Mes_Thiery  0.766
# NK_Exh_Bulk--IL6_JAK_STAT3.currentCoxRes1         Low NK_Exh_Bulk & High IL6_JAK_STAT3  0.828
# CD8NK_Exh_Bulk--TGFb_Signaling.currentCoxRes1 Low CD8NK_Exh_Bulk & High TGFb_Signaling  0.707

cox_OS_2Comb_stage <-
  coxph_multitest(
    data = wide2CombLong_stage,
    variableNameCol = "Comb_Name",
    variableCol = "Comb_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_OS_2Comb_stage_sig <-
  cox_OS_2Comb_stage %>% filter(`Pr(>|z|)` < 0.05) %>%
  data.frame(check.names = F)

# Stage_2group--DDR.currentCoxRes                           I-II Stage_2group & Low DDR  1.070
# Stage_2group--DDR.currentCoxRes1                       III-IV Stage_2group & High DDR  1.705
# Stage_2group--DDR.currentCoxRes2                        III-IV Stage_2group & Low DDR  1.555
# Stage_2group--OXPHOS.currentCoxRes1                 III-IV Stage_2group & High OXPHOS  0.923
# Stage_2group--OXPHOS.currentCoxRes2                  III-IV Stage_2group & Low OXPHOS  1.023
# Stage_2group--TP53_signaling.currentCoxRes1 III-IV Stage_2group & High TP53_signaling  0.795
# Stage_2group--TP53_signaling.currentCoxRes2  III-IV Stage_2group & Low TP53_signaling  0.943
# Stage_2group--HPX_EGF_EMT.currentCoxRes           I-II Stage_2group & Low HPX_EGF_EMT -0.841
# Stage_2group--HPX_EGF_EMT.currentCoxRes2        III-IV Stage_2group & Low HPX_EGF_EMT  0.984
# Stage_2group--Peroxisome.currentCoxRes1         III-IV Stage_2group & High Peroxisome  1.206
# Stage_2group--Peroxisome.currentCoxRes2          III-IV Stage_2group & Low Peroxisome  0.968
# Stage_2group--Mes_Thiery.currentCoxRes2          III-IV Stage_2group & Low Mes_Thiery  1.542
# Stage_2group--Epi_Thiery.currentCoxRes1         III-IV Stage_2group & High Epi_Thiery  1.293
# Stage_2group--IL6_JAK_STAT3.currentCoxRes1   III-IV Stage_2group & High IL6_JAK_STAT3  1.050
# Stage_2group--IL6_JAK_STAT3.currentCoxRes2    III-IV Stage_2group & Low IL6_JAK_STAT3  0.963
# Stage_2group--TGFb_Signaling.currentCoxRes1 III-IV Stage_2group & High TGFb_Signaling  0.918

cox_OS_2Comb_msi <-
  coxph_multitest(
    data = wide2CombLong_msi,
    variableNameCol = "Comb_Name",
    variableCol = "Comb_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_OS_2Comb_msi_sig <-
  cox_OS_2Comb_msi %>% filter(`Pr(>|z|)` < 0.05) %>%
  data.frame(check.names = F)

## MSI_Group--Mes_Thiery.
```

Coef plot for the significant ones:
- None of the CD3 and CD8 combinations were significant.
- MSI was only significant when combined with Mes
- Stage had several significant terms, however, most of it were due to the stage itself...
```{r}

dd <- rbind(cox_OS_2Comb_sig, cox_OS_2Comb_stage_sig, cox_OS_2Comb_msi_sig)
dd <- dd[, c("VariableName",
             "variable",
             "exp(coef)",
             "lower .95",
...
dd <- dd[order(dd$estimate), ]
dd$term <- gsub("_2group", "", dd$term)
dd$term <- gsub("MSI_Group ", "", dd$term)

pdf(
  paste0(figPath, "OS_TCGA_Cox_Benchmark_ScorePairs_Others.pdf"),
  height = 7,
  width = 5
)
ggstatsplot::ggcoefstats(
...
dd <- rbind(cox_OS_2Comb, cox_OS_2Comb_stage, cox_OS_2Comb_msi)
dd <- dd[, c("VariableName",
             "variable",
             "exp(coef)",
             "lower .95",
...
pdf(
  paste0(figPath, "OS_TCGA_Cox_Benchmark_ScorePairs_Others_ALL.pdf"),
  height = 18,
  width = 5
)
...
#### Compare base model with models including score pairs
```{r}
covarCol <- c("Age", "Stage", "CMS", "MSI_TCGAbiolink_annot")
...
base_model <-
  coxph(currentFormula,
        data = wide2Comb[complete.cases(wide2Comb$Stage), ])



cox_OS_2Comb_models <-
  coxph_multitest(
    data = wide2CombLong_scores[complete.cases(wide2CombLong_scores$Stage), ],
    variableNameCol = "Comb_Name",
    variableCol = "Comb_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
anRes_pairs <- sapply(cox_OS_2Comb_models, function(x){
  anovaRes <- anova(base_model, x)[[4]][2]
  return(anovaRes)
})

sigAnova <- names(anRes_pairs[anRes_pairs <= 0.05])
# "NK_Res_Bulk--Mes_Thiery"    
# "CD4_Exh_Bulk--TGFb_Signaling"
# "NK_Exh_Bulk--TGFb_Signaling"   
# "CD8NK_Exh_Bulk--TGFb_Signaling"   

resData <-
  data.frame(matrix(ncol = 3, nrow = 0))
colnames(resData) <- c("pvalue", "Contrast", "Variable")
        
for(i in sigAnova) {
  pairwise_estimate <- multcomp::glht(
    cox_OS_2Comb_models[names(cox_OS_2Comb_models) == i][[1]],
    linfct = multcomp::mcp(Comb_2status = "Tukey")
  )
  x <- summary(pairwise_estimate)
  pvals <- data.frame(pvalue = cbind(x$test$pvalues))
  pvals$Contrast <- names(x$test$coefficients)
  pvals$Variable <- i
  
  resData <- rbind(resData, pvals)
}


resData <- resData[, c(3, 2, 1)]
write.csv(resData, paste0(outPath, "Cox_TCGA_ANOVA_Tukey.csv"), row.names = F)
```


Look at each of these models that have significant anova test in more details
```{r}
cox_OS_2Comb_sigAnova <-
  coxph_multitest(
    data = wide2CombLong[wide2CombLong$Comb_Name %in% sigAnova, ],
    variableNameCol = "Comb_Name",
    variableCol = "Comb_2status",
    timeCol = "OS.time",
    eventCol = "OS",
...
cox_OS_2Comb_sigAnova[cox_OS_2Comb_sigAnova$`Pr(>|z|)` < 0.05, ]

# NK_Res_Bulk--Mes_Thiery.currentCoxRes1               Low NK_Res_Bulk & High Mes_Thiery -1.135
# CD8NK_Exh_Bulk--TGFb_Signaling.currentCoxRes1 Low CD8NK_Exh_Bulk & High TGFb_Signaling  0.707
# Stage_2group--DDR.currentCoxRes                            I-II Stage_2group & Low DDR  1.021
# Stage_2group--DDR.currentCoxRes1                        III-IV Stage_2group & High DDR  2.653
# Stage_2group--DDR.currentCoxRes2                         III-IV Stage_2group & Low DDR  2.484
# Stage_2group--HPX_EGF_EMT.currentCoxRes1        III-IV Stage_2group & High HPX_EGF_EMT  1.478
# Stage_2group--HPX_EGF_EMT.currentCoxRes2         III-IV Stage_2group & Low HPX_EGF_EMT  2.053
# MSI_Group--Mes_Thiery.currentCoxRes1                   MSS MSI_Group & High Mes_Thiery -1.025
```
